    ...
    $.ajax({
      ...
      success: function (response) {
        if (response.status === "success") {
          ...
          $("#pinSection").addClass("d-none");
          $("#memberStatus")
            .text("Membership ID Verified. Enjoy your 10% Discount!")
            .removeClass("text-danger text-muted")
            .addClass("text-success");
          ...
          $("#sendCodeBtn")
            ...
            .addClass("btn-danger")
            .prop("disabled", true);
          $("#firstName, #lastName, #email, #contactNum, #membershipId").attr(
            "readonly",
            true,
          );
    ...
    if (startTime && endTime && rate) {
      $.ajax({
        ...
        success: function (response) {
          if (response.status === "success") {
            $("#total")
              .val(parseFloat(response.total).toFixed(2))
              .attr("readonly", true);
            $("#partial-payment").text(
              `₱${parseFloat(response.total / 2).toFixed(2)}`,
            );
            $("#partialPaymentSection").removeClass("d-none");

            let loadingMsg = isMember
      ...
      $.ajax({
        ...
        success: function (response) {
          if (response.status === "success") {
            ...
            $("#capacity, #amount").attr("readonly", true).trigger("input");
          } else {
            $("#capacity, #amount").val("");
  ...
  $("#court, #date").on("change", function () {
    ...
    $.ajax({
      ...
      success: function (response) {
        ...
        const slots = (response.bookedSlots ?? [])
          .map((s) => ({
        ...
        if (response.status === "success") {
          if (isChecked) {
            Swal.fire({
              ...
              text: response.noBalanceMessage
                ? response.noBalanceMessage
                : "Your wallet balance has been applied to the total amount.",
              timer: 2000,
              showConfirmButton: false,
            });
            ...
            $("#walletText").val(
              parseFloat(response.new_wallet_balance).toFixed(2),
            );
...
$(document).ready(function () {
  $("#btnLookupMember").on("click", function () {
    const cardNum = $("#lookupCardNumber").val();
    const $errorMsg = $("#lookupErrorMessage");
    const $btn = $(this);
    ...
    $.ajax({
      ...
      success: function (response) {
        if (response.success) {
          ...
          // 1. Prepare and Inject the Profile Header (Just like the transaction rows)
          let headerHtml = `
            <div class="p-3 bg-light rounded text-center mb-3">
                <h5 class="mb-1">Welcome, <span class="text-primary">${member.first_name} ${member.last_name}</span>!</h5>
                <p class="mb-1 small text-muted">Current Wallet Balance:</p>
                <h2 class="text-danger mb-0">₱${parseFloat(member.wallet).toLocaleString(undefined, { minimumFractionDigits: 2 })}</h2>
            </div>`;

          $("#profileHeaderInjection").html(headerHtml);

          // 2. Prepare and Inject the Transaction History
          let rows = "";
            ...
            member.transactions.forEach((tx) => {
              rows += `
                    <tr>
                        ...
                        <td class="text-wrap">${tx.item_summary}</td>
                        <td class="text-end fw-bold">₱${parseFloat(tx.final_total).toFixed(2)}</td>
                    </tr>`;
            });
          } else {
            rows =
          ...
          // 3. Show the section
          $("#memberSearchSection").addClass("d-none");
          $("#memberDetailsSection").removeClass("d-none").fadeIn();
        } else {
          $("#lookupErrorMessage")
            .removeClass("d-none")
  ...
  // Reset button logic
  $("#btnResetLookup").on("click", function () {
    $("#memberDetailsSection").fadeOut(200, function () {
      $(this).addClass("d-none");
      $("#memberSearchSection").fadeIn(200);
      $("#lookupCardNumber").val("").focus();
    });
  });
});

$(document).ready(function () {
  // Event listener for the "Check Another Card" button
  $(document).on("click", "#btnResetLookup", function (e) {
    e.preventDefault();

    // 1. Fade out the details section for a smooth transition
    $("#memberDetailsSection").fadeOut(200, function () {
      // 2. Hide the details section and show the search section
      $(this).addClass("d-none");
      $("#memberSearchSection").removeClass("d-none").fadeIn(200);

      // 3. Clear the injected HTML so it's fresh for the next person
      $("#profileHeaderInjection").empty();
      $("#transactionHistoryBody").empty();

      // 4. Clear the input field and put the cursor back inside
      $("#lookupCardNumber").val("").focus();

      // 5. Hide any leftover error messages from the previous attempt
      $("#lookupErrorMessage").addClass("d-none");
    });
    $("#btnLookupMember").prop("disabled", false).text("Confirm");
  });
});

$(document).ready(function () {
  // This triggers whenever the modal is closed (by button, X, or clicking outside)
  $("#accountDetails").on("hidden.bs.modal", function () {
    // 1. Reset the visibility to the search section
    $("#memberDetailsSection").addClass("d-none");
    $("#memberSearchSection").removeClass("d-none").show();

    // 2. Wipe the injected dynamic content
    $("#profileHeaderInjection").empty();
    $("#transactionHistoryBody").empty();

    // 3. Clear the input and errors
    $("#lookupCardNumber").val("");
    $("#lookupErrorMessage").addClass("d-none");

    // 4. Reset the lookup button state if it was stuck on "Checking..."
    $("#btnLookupMember").prop("disabled", false).text("Confirm");
  });
});
